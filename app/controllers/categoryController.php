...
class categoryController extends Controller
{
	...
	function index($vars = null)
	{
		if ($vars['id']) {
			...
			$this->pageUrl = DOMAIN . "category/" . $category['id'] . "/" . $category['name'];
			...
			// If a search is made
			if ($search) {
				$records = $this->productModel->countAllByCategorySearch($search);
				$products = $this->productModel->searchByCategoryWithPaging($search, $fromRecords, $perPage);
			}
		
			// If no search is made
			else {
				$records = $this->productModel->countAllByCategory();
				$products = $this->productModel->readAllByCategoryWithPaging($fromRecords, $perPage);
			}
