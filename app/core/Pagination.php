...
class Pagination
{
	...
	public function paging(
		...
		if ($records > $maxshown) {
			...
			self::$totalPages = ceil(
				intval($records) / ($maxshown >= 1 ? intval($maxshown) : 1)
			);
			...
			$paging =
				'<ul class="' .
				$this->getPaginationClass() .
				'">' .
				$this->preLinks($arrows);
	...
	{
		return "<li" .
			(!empty($this->getLiClass()) ||
			($current === true && !empty($this->getLiActiveClass()))
				? ' class="' .
					trim(
						$this->getLiClass() .
							($current === true &&
							!empty($this->getLiActiveClass())
			...
			'><a href="' .
			self::$pageURL .
			(!empty($this->buildQueryString($link))
				? "?" . $this->buildQueryString($link)
				: "") .
			'" title="Page ' .
			$page .
			'"' .
			(!empty($this->getAClass()) ||
			($current === true && !empty($this->getAActiveClass()))
				? ' class="' .
					trim(
						$this->getAClass() .
							($current === true &&
							!empty($this->getAActiveClass())
								? " " . $this->getAActiveClass()
								: "")
					) .
					'"'
				: "") .
	...
	protected function buildQueryString($page)
	{
		...
		return http_build_query(
			array_filter(array_merge($pageInfo, $this->queryString)),
			"",
			"&amp;"
		);
	}
	...
	protected function getPage($records, $maxshown, $numpages)
	{
		...
		if (self::$lastpage > $numpages) {
			...
			if (self::$current < self::$lastpage - $show) {
				self::$lastpage =
					self::$current <= $show
						? self::$current + ($numpages - self::$current)
						: self::$current + $show;
			} else {
				self::$page =
					self::$current -
	...
	protected function postLinks($arrows = true)
	{
		...
		if (self::$current != self::$totalPages && $arrows) {
			...
			if (self::$current != self::$totalPages - 1) {
				$paging .= $this->buildLink(self::$totalPages, "&raquo;");
			}
		}
		return $paging;
	}
